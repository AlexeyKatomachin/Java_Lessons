...
public class MyServHandl implements HttpHandler {

    private static final Logger logger = Log.create(MyServHandl.class);

    private MessageHistory messageStorage = new MessageHistory();

    private MyResp doPut(HttpExchange httpExchange) {
        try {
            ...
            return new MyResp(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect request body");
        } catch (InvalidTokenException e) {
            logger.error("PUT: invalid token.", e);
            return MyResp.badRequest(e.getMessage());
        }
    }

    private MyResp doDelete(HttpExchange httpExchange) {
        try {
    ...
    private MyResp doOptions(HttpExchange httpExchange) {
    ...
    public void handle(HttpExchange httpExchange) throws IOException {
        ...
        try {
            ...
            response = new MyResp(Constants.RESPONSE_CODE_INTERNAL_SERVER_ERROR, "Error while dispatching message");
        }
        sendResponse(httpExchange, response);

    }

    private MyResp dispatch(HttpExchange httpExchange) {
        if (Constants.REQUEST_METHOD_GET.equals(httpExchange.getRequestMethod())) {
            ...
            return new MyResp(Constants.RESPONSE_CODE_METHOD_NOT_ALLOWED,
                    String.format("Unsupported http method %s", httpExchange.getRequestMethod()));
        }
    }

    private MyResp doGet(HttpExchange httpExchange) {
        try {
            Map<String, String> params = getRequestParams(httpExchange);
    ...
    private MyResp doPost(HttpExchange httpExchange) {
        try {
    ...
    private MyResp respondWithUpdates(Map<String, String> params) throws InvalidTokenException {
        String token = getTokenParam(params);
        int index = MessageHelper.parseToken(token);
        if (index > messageStorage.size()) {
            return MyResp.badRequest(
